...
(function(root){
  ...
  function weighing(value,tenths){
    if(value===undefined)return {};
    if(!value||typeof value!=='object'||Array.isArray(value))invalid('称重记录格式不正确');
    const initialTenths=integer(value.initialTenths,0,100000),endingTenths=integer(value.endingTenths,0,100000);
    if(initialTenths-endingTenths!==tenths)invalid('两次称重差值与本次食量不一致');
    return {weighing:{initialTenths,endingTenths}};
  }
  // Version 4 prevents older clients from silently stripping weighing details on write.
  function empty(){return {app:APP,version:4,month:'2026-09',revision:0,entries:{},meals:[],weights:{},appliedIds:[]};}
  function validate(data,today=dateISO()){
    if(!data||data.app!==APP||![1,2,3,4].includes(data.version)||data.month!=='2026-09'||!data.entries||Array.isArray(data.entries)||typeof data.entries!=='object')invalid('不是有效的妙妙日历备份');
    const out=empty();
    ...
    for(const m of meals){const ident=id(m.id);if(seen.has(ident))invalid('喂食记录重复');seen.add(ident);out.meals.push({id:ident,day:day(m.day),tenths:integer(m.tenths,1,10000),...weighing(m.weighing,m.tenths),recordedAt:stamp(m.recordedAt),deletedAt:m.deletedAt?stamp(m.deletedAt):null});}
  ...
  function apply(source,op,today=dateISO()){
    ...
    switch(op.kind){
      ...
      case 'meal':{
        ...
        const details=weighing(op.weighing,tenths);
        if(!existing)s.meals.push({id:ident,day:op.day,tenths,...details,recordedAt:time,deletedAt:null});break;
      }
      case 'meal-status':{
